...
$(document).on("ready", function() {
    ...
    $('.select-color').on('change', function() {
        $.cookie("themecolor", $(this).attr('data-color'), {
            expires: 7
        });
        $('link[title=styles-animations]').prop('disabled', true);
        $('head').append('<link rel="stylesheet" href="../css/' + $.cookie("themecolor") + '.css" type="text/css" title="styles-animations">');
    ...
    $('.select-reading').on('change', function() {
        $.cookie("themereading", $(this).attr('data-reading'), {
            expires: 7
        });
        if ($(this).attr('data-reading') === 'rtl-read') {
            $('body').addClass("rtl-read");
        } else {
            $('body').removeClass("rtl-read");
        }
        //$('#themepicker').modal('toggle');
    });
    ...
    $('.select-rounded').on('change', function() {
        $.cookie("themerounded", $(this).attr('data-rounded'), {
            expires: 7
        });
        if ($(this).attr('data-rounded') === 'rounded') {
    ...
    if ($(window).width() >= 1440 && body_1.hasClass("no-menu-show") != true) {
        ...
        $(document).on('click', function(event) {
            if (event.target.className !== ".menu-collapse,.menu-collapse-right, .sidebar-left,.sidebar-right") {
                body_1.addClass('menuclose menuclose-right');
            }
        });
        $('.menu-collapse,.menu-collapse-right, .sidebar-left,.sidebar-right').on('click', function(event) {
